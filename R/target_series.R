...
#'     cls_2 <- class_targets("class 2", AREA=c(0, 1000))
#'     ls_targets <- landscape_targets(200, 200, list(cls_1, cls_2))
#'     target_series <- target_series(ls_targets, class_name="class 2",
#'                                    target_key="NP", sequence=seq(1, 10, by=1))
#'   }
#' @export
#'
target_series <- function(landscape_targets, class_name=NULL, class_id=NULL,
                          target_key, sequence) {
