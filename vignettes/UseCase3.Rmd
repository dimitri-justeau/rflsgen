...
struct <- flsgen_load_structure_from_raster(path, c(0, 1), connectivity = 8)
```

The second argument corresponds to the focal classes raster values (0 is open forest, and 1 closed forest), and for this use case we use the 8-connectivity rule. We can print information about the landscape structure by converting the json structure to a dataframe:

```{r}
df_struct <- jsonlite::fromJSON(struct)
df_struct$classes$NP
df_struct$classes$PLAND
```
